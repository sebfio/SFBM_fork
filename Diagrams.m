...
% Arrow function
DrawArrow = @(x,y,varargin) quiver( x,y(1),0,y(2)-y(1),0, varargin{:} ) ;
...
for n   = 1:nc
    ...
    if yl1 ~= yl2
        if TypeF(n) == 'a';
            if (nca == 0)
                nca = 1;
                hca = DrawArrow(xl,[yl1,yl2],'linewidth',1.5,'color','k','MaxHeadSize',10);
                PlotHandles = [PlotHandles; hca]; PlotNames = [PlotNames;{'Applied Concentrated Force'}];
            else
                DrawArrow(xl,[yl1,yl2],'linewidth',1.5,'color','k','MaxHeadSize',10);
            end
        else
            if (ncr == 0)
                ncr = 1;
                hcr = DrawArrow(xl,[yl1,yl2],'linewidth',1.5,'color','k','MaxHeadSize',10, 'linestyle','-.');
                PlotHandles = [PlotHandles; hcr];  PlotNames = [PlotNames;{'Reacting Concentrated Force'}];
            else
                DrawArrow(xl,[yl1,yl2],'linewidth',1.5,'color','k','MaxHeadSize',10, 'linestyle','-.');
            end
        end
    end
end
    ...
    for nn = 1:num
        if ydist2(nn) ~= ydist1(nn)
            if TypeD(n) == 'a';
                if (nda == 0)
                    nda = 1;
                    hda = DrawArrow(xdist(nn),[ydist1(nn), ydist2(nn)],'linewidth',1.5,'color','m','MaxHeadSize',10);
                    PlotHandles = [PlotHandles; hda];  PlotNames = [PlotNames;{'Applied Distributed Force'}];
                else
                    DrawArrow(xdist(nn),[ydist1(nn), ydist2(nn)],'linewidth',1.5,'color','m','MaxHeadSize',10);
                end
            else
                if (ndr == 0)
                    ndr = 1;
                    hdr = DrawArrow(xdist(nn),[ydist1(nn), ydist2(nn)],'linewidth',1.5,'color','m','LineStyle','-.','MaxHeadSize',10);
                    PlotHandles = [PlotHandles; hdr];  PlotNames = [PlotNames;{'Reacting Distributed Force'}];
                else
                    DrawArrow(xdist(nn),[ydist1(nn), ydist2(nn)],'linewidth',1.5,'color','m','LineStyle','-.','MaxHeadSize',10);
                end
            end
        end
    end
